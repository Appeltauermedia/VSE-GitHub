...
function clampScreenPlaylistHold(value){return Math.max(.5,Math.min(180,Number(value)||5));}
function formatScreenPlaylistHold(value){
  const seconds=clampScreenPlaylistHold(value);
  if(seconds>=60){
...
function updateScreenPlaylistHoldLabel(value){
  if(screenPlaylistHoldValue)screenPlaylistHoldValue.textContent=formatScreenPlaylistHold(value);
}
function renderScreenPlaylistEditor(o){
  if(!screenPlaylistEditor)return;
  if(!o||o.type!=='screen'){screenPlaylistEditor.innerHTML='<div class="mini">Keine Playlist-Medien.</div>';return;}
  const list=typeof normalizeScreenPlaylist==='function'?normalizeScreenPlaylist(o):(o.screenPlaylist||[]);
  if(!list.length){screenPlaylistEditor.innerHTML='<div class="mini">Keine Playlist-Medien.</div>';return;}
  const total=typeof screenPlaylistTotalDuration==='function'?screenPlaylistTotalDuration(o):list.reduce((max,item)=>Math.max(max,(Number(item.start)||0)+(Number(item.duration)||0)),0);
  const esc=s=>String(s??'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
  screenPlaylistEditor.innerHTML='<div class="mini">Playlist gesamt: '+total.toFixed(2)+' s</div>'+list.map((item,index)=>{
    const active=index===(Number(o.screenPlaylistIndex)||0);
    return '<div class="screenPlaylistRow '+(active?'isActive':'')+'" data-index="'+index+'">'
      +'<div class="screenPlaylistName">'+esc(item.name||'Medium')+'</div>'
      +'<label>Start<input class="screenPlaylistStart" type="number" min="0" step="0.01" value="'+Number(item.start||0).toFixed(2)+'"></label>'
      +'<label>Dauer<input class="screenPlaylistDuration" type="number" min="0.1" step="0.01" value="'+Number(item.duration||0).toFixed(2)+'"></label>'
      +'<button type="button" data-action="remove">Entfernen</button>'
      +'</div>';
  }).join('');
  screenPlaylistEditor.querySelectorAll('.screenPlaylistRow').forEach(row=>{
    const index=Number(row.dataset.index)||0;
    const startInput=row.querySelector('.screenPlaylistStart');
    const durationInput=row.querySelector('.screenPlaylistDuration');
    const apply=()=>{
      const item=o.screenPlaylist&&o.screenPlaylist[index];
      if(!item)return;
      item.start=Math.max(0,Number(String(startInput.value||'').replace(',','.'))||0);
      item.duration=Math.max(.1,Number(String(durationInput.value||'').replace(',','.'))||.1);
      o.screenPlaylist.sort((a,b)=>(Number(a.start)||0)-(Number(b.start)||0));
      if(typeof updateTimelineUI==='function')updateTimelineUI();
      renderScreenPlaylistEditor(o);
    };
    if(startInput)startInput.addEventListener('change',apply);
    if(durationInput)durationInput.addEventListener('change',apply);
    const remove=row.querySelector('button[data-action="remove"]');
    if(remove)remove.addEventListener('click',()=>{
      o.screenPlaylist.splice(index,1);
      if(o.screenPlaylistIndex>=o.screenPlaylist.length)o.screenPlaylistIndex=o.screenPlaylist.length-1;
      if(o.screenPlaylistIndex<0)releaseScreenMedia(o);else loadScreenPlaylistItem(o,o.screenPlaylistIndex,true);
      if(typeof updateTimelineUI==='function')updateTimelineUI();
      select(o);
    });
  });
...
function selectSingleCore(o){
  ...
  if(typeof syncObjectParamFloatingSelection==='function')syncObjectParamFloatingSelection();
  if(typeof syncInputManagerPanel==='function')syncInputManagerPanel();
  ...
  if(fields.ScreenCornerPerspective)fields.ScreenCornerPerspective.checked=!!o.screenCornerPerspective;
  ...
  if(fields.ImageAssetLocked)fields.ImageAssetLocked.checked=!!o.imageAssetLocked;
  ...
  if(fields.ParticlePathShape)fields.ParticlePathShape.value=o.particlePathShape||'line';
  if(fields.ParticlePathWidth)fields.ParticlePathWidth.value=o.particlePathWidth??360;
  if(fields.ParticlePathHeight)fields.ParticlePathHeight.value=o.particlePathHeight??180;
  if(fields.ParticlePathArc)fields.ParticlePathArc.value=o.particlePathArc??180;
  if(fields.ParticlePathFlow)fields.ParticlePathFlow.value=o.particlePathFlow??1;
  if(fields.ParticlePathSpread)fields.ParticlePathSpread.value=o.particlePathSpread??18;
  if(fields.ParticleAltColor2)fields.ParticleAltColor2.value=o.particleAltColor2||'#ff4a18';
  ...
  if(screenMediaInfo)screenMediaInfo.textContent=o.screenPlaylist&&o.screenPlaylist.length?('Playlist: '+o.screenPlaylist.length+' Medien · gesamt: '+(typeof screenPlaylistTotalDuration==='function'?screenPlaylistTotalDuration(o).toFixed(2):'0.00')+' s · aktuell: '+(o.screenMediaName||'—')):(o.screenMediaName?('Geladen: '+o.screenMediaName+' · '+(o.screenCaptureStream?'Live-Aufnahme':(o.screenMediaType||'Medium'))+(o.screenMediaType==='video'&&!o.screenMediaElement&&!o.screenCaptureStream?' · Video nicht eingebettet':'')):'Kein Medium geladen.');
  if(screenPlaylistHold)screenPlaylistHold.value=clampScreenPlaylistHold(o.screenPlaylistHold??5);
  if(screenPlaylistAuto)screenPlaylistAuto.checked=o.screenPlaylistAuto!==false;
  updateScreenPlaylistHoldLabel(o.screenPlaylistHold??5);
  renderScreenPlaylistEditor(o);
  ...
  updateAudioFreqUI(o);
  if(fields.Life)fields.Life.value=o.life;
  syncTypeUI();
  if(typeof syncObjectParamFloatingSelection==='function')syncObjectParamFloatingSelection();
...
function syncTypeUI(){
  ...
  const isInputManager=selected&&selected.type==='inputManager';
  ...
  document.querySelectorAll('.type-inputmanager-only').forEach(e=>e.style.display=isInputManager?'block':'none');
  ...
  if(selected){
    ...
    updateScreenPlaylistHoldLabel(selected.screenPlaylistHold??5);
    ...
    if(typeof particlePathWidthValue!=='undefined')particlePathWidthValue.textContent=Math.round(Number(selected.particlePathWidth??360));
    if(typeof particlePathHeightValue!=='undefined')particlePathHeightValue.textContent=Math.round(Number(selected.particlePathHeight??180));
    if(typeof particlePathArcValue!=='undefined')particlePathArcValue.textContent=Math.round(Number(selected.particlePathArc??180))+'°';
    if(typeof particlePathFlowValue!=='undefined')particlePathFlowValue.textContent=Number(selected.particlePathFlow??1).toFixed(2);
    if(typeof particlePathSpreadValue!=='undefined')particlePathSpreadValue.textContent=Math.round(Number(selected.particlePathSpread??18));
...
function applyTypeDefaults(o,type){
  ...
  if(type==='particle'){
    ...
    o.particleAltColor2=o.particleAltColor2||'#ff4a18';
    o.particlePathShape=o.particlePathShape||'line';
    o.particlePathWidth=o.particlePathWidth??360;
    o.particlePathHeight=o.particlePathHeight??180;
    o.particlePathArc=o.particlePathArc??180;
    o.particlePathFlow=o.particlePathFlow??1;
    o.particlePathSpread=o.particlePathSpread??18;
    if(!Array.isArray(o.particlePathPoints))o.particlePathPoints=[];
  ...
  if(type==='inputManager'){
    o.color=o.color||'#75dfff'; o.size=o.size||58; o.inputManagerDeviceId=o.inputManagerDeviceId||'keyboard'; o.inputManagerDeviceKind=o.inputManagerDeviceKind||'keyboard'; o.inputManagerDeviceLabel=o.inputManagerDeviceLabel||'Tastatur'; o.inputManagerLastInput=o.inputManagerLastInput||null; o.inputManagerListenKeyboard=o.inputManagerListenKeyboard!==false; o.inputManagerListenPointer=o.inputManagerListenPointer!==false; o.music=o.music??0;
  }
  if(type==='greenscreen'){
...
Object.entries(fields).forEach(([k,el])=>el&&el.addEventListener('input',()=>{
  ...
  syncTypeUI();
  if(selected&&selected.type==='particle'&&['particleEmitterShape','particleEmitterLength'].includes(map[k])&&typeof showTransformFrameForSelection==='function')requestAnimationFrame(showTransformFrameForSelection);
  if(selected&&selected.type==='screen'&&['screenCornerPerspective','screenWidth','screenHeight'].includes(map[k])&&typeof showTransformFrameForSelection==='function')requestAnimationFrame(showTransformFrameForSelection);
}));
...
if(screenMediaFolder)screenMediaFolder.addEventListener('change',()=>{if(selected&&selected.type==='screen'&&screenMediaFolder.files&&screenMediaFolder.files.length){loadScreenMediaFolder(selected,screenMediaFolder.files);renderScreenPlaylistEditor(selected);if(typeof updateTimelineUI==='function')updateTimelineUI();}screenMediaFolder.value='';});
if(screenPlaylistFiles)screenPlaylistFiles.addEventListener('change',()=>{if(selected&&selected.type==='screen'&&screenPlaylistFiles.files&&screenPlaylistFiles.files.length&&typeof appendScreenPlaylistFiles==='function'){appendScreenPlaylistFiles(selected,screenPlaylistFiles.files,false);renderScreenPlaylistEditor(selected);if(typeof updateTimelineUI==='function')updateTimelineUI();}screenPlaylistFiles.value='';});
if(screenPlaylistHold)screenPlaylistHold.addEventListener('input',()=>{if(!selected||selected.type!=='screen')return;const value=clampScreenPlaylistHold(screenPlaylistHold.value);screenPlaylistHold.value=value;getBulkTypeTargets('screen').forEach(o=>o.screenPlaylistHold=value);updateScreenPlaylistHoldLabel(value);syncTypeUI();});
...
function syncBackgroundPanUi(){
  ...
  if(bgPanX)bgPanX.value=background.panX;
  if(bgPanY)bgPanY.value=background.panY;
  const xValue=document.getElementById('bgPanXValue'),yValue=document.getElementById('bgPanYValue');
  if(xValue)xValue.textContent=Math.round(background.panX*100)+'%';
  if(yValue)yValue.textContent=Math.round(background.panY*100)+'%';
}
if(bgPanX)bgPanX.addEventListener('input',()=>{background.panX=parseFloat(bgPanX.value)||0;syncBackgroundPanUi();});
if(bgPanY)bgPanY.addEventListener('input',()=>{background.panY=parseFloat(bgPanY.value)||0;syncBackgroundPanUi();});
bgFile.addEventListener('change',()=>{const f=bgFile.files&&bgFile.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{const img=new Image();img.onload=()=>{background.imageName=f.name;background.imageData=r.result;background.mode='cover';background.panX=0;background.panY=0;if(bgMode)bgMode.value='cover';syncBackgroundPanUi();bgImageData=r.result;makeBgTexture(img);applyBackgroundSizeToStage(img);};img.src=r.result;};r.readAsDataURL(f);});
clearBgBtn.onclick=()=>{background.imageName=null;background.imageData=null;background.panX=0;background.panY=0;syncBackgroundPanUi();bgImageData=null;if(bgTex){gl.deleteTexture(bgTex);bgTex=null;}bgImageSize=[0,0];bgFile.value='';};
