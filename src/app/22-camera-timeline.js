...
(()=>{
  ...
  const currentView=()=>({
    zoom:Math.max(.2,Math.min(4,Number(scene.cameraZoom)||1)),
  ...
  panel.innerHTML=`
    ...
    <div class="cameraTimelineBody">
      ...
      <div class="cameraViewBlock cameraCurveBlock"><b>Pfadkurve</b><span>Kurve als Pixel-Offset auf die Kamerafahrt legen.</span>
        <label>Kurvenform</label><select id="cameraPathType"><option value="linear">Gerade</option><option value="sine">Sinuskurve</option><option value="circle">Kreisförmiger Kamerazoom</option></select>
  ...
  const waveSettings=byId('cameraWaveSettings'),circleSettings=byId('cameraCircleSettings');
  ...
  let editingId='',startView=wholeView(),endView=wholeView();
  ...
  function refreshPanel(){
    ...
    waveSettings.hidden=pathType.value!=='sine';circleSettings.hidden=pathType.value!=='circle';
    saveButton.textContent=editingId?'Kamerafahrt aktualisieren':'Kamerafahrt hinzufügen';
    deleteButton.disabled=!editingId;
  }
  function applyView(view){
    scene.cameraZoom=Math.max(.2,Math.min(4,Number(view.zoom)||1));
  ...
  function openPanel(move=null){
    ...
    pathType.value=move&&['sine','circle'].includes(move.cameraPathType)?move.cameraPathType:'linear';
  ...
  byId('cameraCaptureStart').onclick=()=>{startView=currentView();refreshPanel();};
  byId('cameraCaptureEnd').onclick=()=>{endView=currentView();refreshPanel();};
  byId('cameraWholeStart').onclick=()=>{startView=wholeView();refreshPanel();};
  byId('cameraWholeEnd').onclick=()=>{endView=wholeView();refreshPanel();};
  ...
  saveButton.onclick=()=>{
    ...
    move.cameraPathType=['sine','circle'].includes(pathType.value)?pathType.value:'linear';
  ...
  function makeDraggable(element,move){
    element.addEventListener('pointerdown',event=>{
      ...
      const onMove=moveEvent=>{
        ...
        move.time=clampTime(originTime+(moveEvent.clientX-originX)/Math.max(1,rect.width)*timelineDuration());
  ...
  function renderCameraTrack(){
    ...
    cameraMoves().forEach((move,index)=>{
      ...
      element.onclick=event=>{
        ...
        timelineState.selectedCameraMoveId=move.id;renderCameraTrack();openPanel(move);
      };
  ...
  function interpolatedView(time){
    ...
    const rawT=Math.max(0,Math.min(1,(time-start)/duration));
    const type=['sine','circle'].includes(active.cameraPathType)?active.cameraPathType:'linear';
  ...
  window.vseCameraTimeline={openMove:move=>openPanel(move),close:closePanel};
