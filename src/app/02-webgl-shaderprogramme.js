...
const VSE_SCREEN_DEPTH_FRAGMENT_SHADER=VSE_SCREEN_FRAGMENT_SHADER
  ...
  float depthSin=sin(depthAngle);
  float normalizedProjectedX=q.x/max(halfSize.x,1.0);
  // Inverse einer planaren perspektivischen Abbildung (Homographie).
  // Ein gemeinsamer linearer Nenner für X und Y hält jede Gerade gerade.
  float perspectiveDenominator=max(0.04,depthCos-normalizedProjectedX*depthSin*0.20);
  float sourceX=q.x/perspectiveDenominator;
