...
(function(){
  ...
  if(open)open.addEventListener('click',e=>{
    e.preventDefault();
    const floating=document.getElementById('objectParamFloatingPanel');
    const floatingButton=document.getElementById('objectParamMenuBtn');
    if(floating){floating.hidden=false;floating.classList.remove('isMinimized');}
    if(floatingButton)floatingButton.setAttribute('aria-expanded','true');
    const container=panel&&panel.closest('details.menuTier');
    if(container)container.open=true;
    if(panel){panel.hidden=false;panel.scrollIntoView({block:'nearest'});}
    document.body.classList.add('cloudExpertOpen');
    if(selected&&selected.type==='cloud')syncCloudUI(selected);
  });
