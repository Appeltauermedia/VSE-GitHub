...
(function(){
  ...
  const cloudLoc={index:gl.getAttribLocation(cloudProg,'aIndex'),pixelRes:U('uPixelRes'),cssRes:U('uCssRes'),origin:U('uOriginCss'),size:U('uSize'),rot:U('uRot'),time:U('uTime'),density:U('uDensity'),softness:U('uSoftness'),brightness:U('uBrightness'),seed:U('uSeed'),motion:U('uMotion'),speed:U('uSpeed'),detail:U('uDetail'),opacity:U('uOpacity'),color:U('uColor'),wind:U('uWind'),audio:U('uAudio'),preset:U('uPreset')};
  const CLOUD_PARTICLE_MAX=256,cloudParticleBuffer=gl.createBuffer();
  ...
  function drawCloud(o){
    ...
    const sz=su(o.size||180);gl.uniform1f(cloudLoc.size,sz*1.55);gl.uniform1f(cloudLoc.rot,Number(o.rotation||0)*Math.PI/180);gl.uniform1f(cloudLoc.time,performance.now()/1000);
    ...
    const col=hex(o.cloudColor);gl.uniform3f(cloudLoc.color,col[0],col[1],col[2]);gl.uniform2f(cloudLoc.wind,Number(wind.cssX||0),Number(wind.cssY||0));gl.uniform1f(cloudLoc.audio,av);gl.uniform1i(cloudLoc.preset,presetIndex(o.cloudPreset));
    const particleCount=Math.max(56,Math.min(CLOUD_PARTICLE_MAX,Math.round(72+Number(o.cloudDetail||0)*144+Number(density||0)*24)));
