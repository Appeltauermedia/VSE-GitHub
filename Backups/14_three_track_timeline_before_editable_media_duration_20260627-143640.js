...
(function(){
  ...
  function reconcileSceneDurations(){
    const scenes=(timelineState.events||[]).filter(event=>event&&event.timelineAssetKind==='scene').sort((a,b)=>(Number(a.time)||0)-(Number(b.time)||0));
    scenes.forEach((event,index)=>{
      const next=scenes[index+1];
      event.duration=next?Math.max(.001,(Number(next.time)||0)-(Number(event.time)||0)-.001):0;
    });
  }
  function normalizeScreenMediaDurations(){
    for(const event of (timelineState.events||[])){
      if(!event||event.timelineAssetKind!=='screen-media')continue;
      const target=objects.find(object=>object&&object.id===timelineEventTargetId(event));
      if(target&&target.type==='screen'&&target.screenMediaType!=='video')event.duration=5;
    }
  }
  ...
  function renderScreenMediaTrack(){
    ...
    mediaEvents.forEach(ev=>{
      ...
      const eventDuration=(Number(ev.duration)||0)>0?Number(ev.duration):(ev.timelineAssetKind==='scene'?Math.max(.1,dur-(Number(ev.time)||0)):0);
