...
(function(){
  ...
  function isScreenMediaEvent(ev){
    if(!ev||!ev.snapshot||ev.timelineAssetKind==='scene')return false;
  ...
  async function addSceneFileToTimeline(file){
    ...
    if(!data||(!Array.isArray(data.objects)&&!data.object))throw new Error('Die Datei enthält keine importierbaren Scene-Objekte.');
    ...
    const start=nextFreeStart('objects',currentTimelineTime(),.1);
